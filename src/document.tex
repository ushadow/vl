...
have previously been neglected, including handling both gestures that are
characterized by their dynamic paths and gestures characterized by their hand
poses; and determining how and when the system should respond to gestures. Our probabilistic recognition framework based on hidden
...
contains the two forms of gestures, and propose a new performance
metric that for evaluating gesture recognition methods for real-time
interaction.
...
hand pose. For example, a \textit{swipe left} gesture is characterized by a
right to left motion, while a \textit{circle} gesture is characterized by a
...
For example, a user may use a \textit{point} hand pose and move around to point
at different things on a display.
...
as an \textit{event} \cite{wobbrock09}. One example is the \textit{wave} gesture
...
\textit{point} gesture, where we want to show the cursor on the screen
...
We develop our system based on the above understanding of the gestures for
...
efforts have used hidden Markov models (HMMs) for gesture recognition with
distinct paths \cite{Starner95, sharma00}. More recently, discriminative models such as
...
Our method is similar to theirs but we consider different forms of gestures in
a single framework.
...
one of the popular ones. Based on the observation of the current interaction
...
Many public data sets for evaluating gesture recognition contain only one form
...
handled by the same method as the dynamic gestures. This dataset does not
contain gestures with distinct hand poses but arbitrary movement
...
mapped image. We compute histogram of oriented gradients (HOGs)
feature \cite{dalal05} from the normalized hand image (cell size =
...
current most likely gesture, and keep and propagate beliefs as probabilities as time progresses.
...
one for gestures with an arbitrary number of repetitions (e.g., \textit{wave}
...
training, we directly compute the maximum likelihood estimates for the mixture of Gaussians emission probabilities for $s_{\text{pose}}$. Let $\underline{x}_1^T$ be a sequence of
...
\begin{figure}[t]
\centering
\includegraphics[width=1.1\columnwidth]{fig/circle_shake_label.ps}
\caption{Most likely hidden states using fixed-lag smoothing. Different
colors indicate different hidden state. Yellow indicates rest position.}
...
the wrong inference does not matter. After a few more frames, estimates are
updated to have the correct most likely gesture label and the system will
respond correct when it detects the start of the post-stroke of ``circle''
gesture.
...
according to the text prompts on screen indicating the name of the gesture to perform.
...
that ignore timing of recognition is inappropriate for real-time
...
$T_{{\text{gts\_stop\_post}}}$ be the ground truth stop time of the post-stroke
phase.
An recognized event is considered a true positive (TP) if the time of response ($T_{\text{response}}$) 
...
For discrete flow gestures, we also define Responsiveness Score (RS) as the time
difference (in seconds) between the moment when the system responds and the moment when the hand goes to a rest
...
For continuous flow gestures, the system responds frame by frame, so it is
more appropriate to use frame-based evaluation. For all the frames that are
recognized at a continuous gestures, we can compute the number of TPs by
...
evaluation metrics we propose in the previous sections. The evaluation is based
...
test sequence. This highlights the challenges in the test sequences: 21
gestures in each continuous unsegmented sequence; sometimes gestures immediately
follow one anther.
...
as training examples and the last 2 sessions of recording as testing examples.
We report the average results for 10 users below.

\begin{figure}[t]
\centering
\includegraphics[trim=10mm 10mm 10mm 0, clip,
width=\columnwidth]{fig/recog_result_m3.eps}
\caption{Visualization of recognition result with respect to ground truth.
...
length is shown at the time of response. For continuous flow gestures, the recognized gesture is shown for each frame
indicating frame-by-frame responses.}
\label{fig:recog-result}
\end{figure}

\begin{itemize}
\item \textbf{Compare different topologies}
In our unified HMM-based framework, is it necessary to
treat gestures with static hand poses differently from the gestures with distinct paths?
...
phases. The second row is the result from using a single state for gestures
with distinct hand poses. Having different HMM topologies for the two forms of
gestures significantly increases the precision, recall and F1 score for gestures with
distinct hand poses.

\begin{table*}[t]
\caption{Results for using different topologies. The numbers in parentheses are
standard deviations.}
\label{tab:result}
\centering
...
\\
\hline
Different topologies for the forms of gestures & 0.77 (0.15) & 0.88 (0.11) &
...
Within the same user, there may be variation in the hand pose for a
...
sacrifice too much delay to reach the limit. Our result shows that the
responsiveness score stays around 0.6-0.7 seconds as we increases $L$.
...
Our system is fast to train as well. Average training time per user is about 5s
with 6 training samples for 7 gestures.
...
The experiment evaluation shows promising results for our unified
probabilistic framework for real-time gesture recognition that handles two forms
of gestures.
The performance for gestures with distinct hand poses is lower than that for
gestures with distinct paths. The reason could be the limitation of both the
pixel and the depth resolutions of the Kinect sensor. At 1.5m, the depth
resolution is about 5mm \cite{khoshelham2011}. It would be interesting to test
the new version of the Kinect sensor which uses a time-of-flight depth sensor
and is reported to have higher resolution. We are going to to explore other
feature descriptors and encoding method (e.g., sparse PCA) to see wether we can
...
We also collected a new dataset that include two forms of gestures, a
combination currently lacking in the community. We are planning to make it
public after anonymizing the participants. We also propose a evaluation metric
that is more pertinent to real-time interaction and different types of gestures.
